...
install_pteroq() {
  ...
  curl -o /etc/systemd/system/panel.service "$GITHUB_URL"/configs/panel.service

  case "$OS" in
  debian | ubuntu)
    sed -i -e "s@<user>@www-data@g" /etc/systemd/system/panel.service
    ;;
  rocky | almalinux)
    sed -i -e "s@<user>@nginx@g" /etc/systemd/system/panel.service
    ;;
  esac
...
configure_nginx() {
  ...
  curl -o "$CONFIG_PATH_AVAIL"/panel.conf "$GITHUB_URL"/configs/$DL_FILE

  sed -i -e "s@<domain>@${FQDN}@g" "$CONFIG_PATH_AVAIL"/panel.conf

  sed -i -e "s@<php_socket>@${PHP_SOCKET}@g" "$CONFIG_PATH_AVAIL"/panel.conf

  case "$OS" in
  ubuntu | debian)
    ln -sf "$CONFIG_PATH_AVAIL"/panel.conf "$CONFIG_PATH_ENABL"/panel.conf
    ;;
  esac
